...
/// BookingStatus, as the API spells it.
enum BookingStatus {
  requested('requested'),
  confirmed('confirmed'),
  inProgress('inProgress'),
  completed('completed'),
  cancelledByFamily('cancelledByFamily'),
  cancelledByCaregiver('cancelledByCaregiver'),
  noShow('noShow');

  const BookingStatus(this.wireName);
  ...
  static BookingStatus fromJson(String value) => values.firstWhere(
    (e) => e.wireName == value,
    orElse: () => throw CareBridgeUnknownEnumValue('BookingStatus', value),
  );
}
