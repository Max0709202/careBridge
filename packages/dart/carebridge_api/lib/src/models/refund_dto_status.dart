...
/// RefundDtoStatus, as the API spells it.
enum RefundDtoStatus {
  pending('pending'),
  succeeded('succeeded'),
  failed('failed');

  const RefundDtoStatus(this.wireName);
  ...
  static RefundDtoStatus fromJson(String value) => values.firstWhere(
    (e) => e.wireName == value,
    orElse: () => throw CareBridgeUnknownEnumValue('RefundDtoStatus', value),
  );
}
